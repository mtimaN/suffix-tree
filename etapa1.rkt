...
(define (longest-common-prefix-of-list words)
  (if (null? words)
      `()
      (if (or (= (length words) 1) (= (length (car words)) 1))
          (car words)
          (longest-common-prefix-of-list (cons (car (longest-common-prefix (car words) (cadr words))) (cddr words))))))
...
(define (get-first-match st pattern)
  (car (get-ch-branch st (car pattern))))

(define (match-pattern-with-label st pattern)
  (cond
    ((equal? (get-ch-branch st (car pattern)) #f) `(#f ()))
    ((equal? (car (longest-common-prefix (get-first-match st pattern) pattern)) pattern) #t)
    ((equal? (car (longest-common-prefix (get-first-match st pattern) pattern)) (get-first-match st pattern))
     (list (car (longest-common-prefix (get-first-match st pattern) pattern))
       (drop pattern (length (car (longest-common-prefix (get-first-match st pattern) pattern))))
       (cdr (get-ch-branch st (car pattern)))))
    (else
     (cons #f (cons (car (longest-common-prefix (get-first-match st pattern) pattern)) `() ))))
  )
